...
then
    ...
    RES=-1
    PING_TIMEOUT=60
    SSL_SERVER_ENABLED_orig=$(grep server.ssl.enabled /usr/share/kibana/config/kibana.yml | cut -d\  -f2)
    ...
    elastic_url_orig=$(grep elasticsearch.host /usr/share/kibana/config/kibana.yml | cut -d\  -f2)
